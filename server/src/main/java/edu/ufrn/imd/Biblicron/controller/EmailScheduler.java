    ...
    */
    public void enviarEmailsDeLembrete() {
        ...
        for (Emprestimo emprestimo : emprestimosProximosDoVencimento) {
            ...
            String assunto = "Lembrete de devolução do Produto " + emprestimo.getProduto().getNomeProduto();
            String mensagem = "Prezad@, " + emprestimo.getUsuario().getUsername() + ".\n" +
                    "\n" +
                    "Gostaríamos de informar que a data limite para devolução do livro " + emprestimo.getProduto().getNomeProduto() +
